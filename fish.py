...
def fish_meal():
    ...
    TEXT_fish = open('data/fish.txt', 'rb').read().decode(encoding='utf-8')
    ...
    model.load_weights("models/fish_model.h5")
    # model.load_weights(tf.train.latest_checkpoint(checkpoint_dir))
    model.build(tf.TensorShape([1, None]))
    text_file_f = generate_text(model, start_string=u"fish ",t=0.1)
    # text_file_m
